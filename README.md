## About Me

I’m passionate about building high-performance systems and AI infrastructure that scale.  
Intersection of **systems engineering**, **machine learning**, **game dev**, and **backend infrastructure**.

---

## Interests

- **Distributed Infra / Systems** – low-latency data pipelines
- **Applied ML Infra** – model deployment, fine-tuning, automated training
- **Backend Engineering** – Flask/Django REST APIs, GCP, containerized workflows  
- **Performance Optimization** – concurrency, caching, task queues, scalability

<!--
---

![](https://komarev.com/ghpvc/?username=jmo-on&style=flat&color=gray)
![Jin Hong's GitHub Stats](https://github-readme-stats.vercel.app/api?username=jmo-on&show_icons=true&theme=default&hide_title=true&hide_rank=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=jmo-on&layout=compact&theme=default)
-->

---

## Experience

**Bloomberg LP** — *Software Engineering Intern (New York, 2025)*  

**Claudius Legal Intelligence** — *Software Engineering Intern (Remote, 2024)*  

---

## Projects

**[Summit](https://summit-jobs.com)** — AI-powered job application tracker  

**[Noori AI](https://nooriai.com)** — Medical speech-to-speech translator

**[Delineo Disease Modeling](https://github.com/Delineo-Disease-Modeling/Algorithms)** — Pandemic simulation research

**[HopHacks](https://github.com/HopHacks/hophacks-flask)** — JHU’s annual hackathon platform  

[![Summit](https://img.shields.io/badge/Summit-Django%20%7C%20Supabase-blue)](https://summit-jobs.com)
[![Noori AI](https://img.shields.io/badge/Noori%20AI-MarianNMT%20%7C%20PyTorch-green)](https://nooriai.com)
[![Delineo](https://img.shields.io/badge/Delineo%20Modeling-Optimization%20%7C%20WHO%20Project-orange)](https://github.com/Delineo-Disease-Modeling/Algorithms)
[![HopHacks](https://img.shields.io/badge/HopHacks-Flask%20%7C%20React-yellow)](https://github.com/HopHacks/hophacks-flask)
